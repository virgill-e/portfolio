<template>
    <button ref="buttonRef"
        class="relative overflow-hidden px-16 py-4 rounded-full border-2 text-custom-forest border-custom-forest uppercase transition-all duration-300 group">
        <span ref="fillRef" class="absolute left-0 top-0 h-full w-0 bg-custom-forest z-0 transition-none"></span>
        <span class="relative z-10 group-hover:text-custom-cream transition-colors duration-300">{{ text }}</span>
    </button>
</template>
